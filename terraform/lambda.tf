...
data "archive_file" "lambdas_clients" {
  for_each = local.lambdas

  output_path = "files/${each.key}_lambdas_clients.zip"
  type        = "zip"
  source_file = "${local.lambdas_path}/lambdas_clients/${each.key}.js"
}

resource "aws_lambda_function" "lambdas_clients" {
  for_each = local.lambdas
  ...
  filename         = data.archive_file.lambdas_clients[each.key].output_path
  source_code_hash = data.archive_file.lambdas_clients[each.key].output_base64sha256
...
resource "aws_lambda_permission" "api" {
  ...
  function_name = aws_lambda_function.lambdas_clients[each.key].arn
